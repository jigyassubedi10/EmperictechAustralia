import PrivacyPolicyBreadcarumb from "@/components/sections/privacy-policy/PrivacyPolicyBreadcarumb";
import getPrivacyPolicy from "@/libs/getPrivacyPolicy";
import Link from "next/link";

export default function PrivacyPolicy() {
  const privacyPolicy = getPrivacyPolicy();
  const { title, description, sections } = privacyPolicy || {};

  return (
    <div>
      {/* Breadcrumb */}
      <PrivacyPolicyBreadcarumb />
      ...
      <section className="container mx-auto py-16 px-6">
          ...
          <div className="space-y-8">
            {sections.map((item, idx) => (
              ...
              >
                ...
                {/* If section has paragraph content */}
                {item.content && (
        ...
        <div className="bg-gradient-primary text-white text-center py-10 px-6 rounded-lg shadow-lg mt-20">
          ...
          <p className="text-lg md:text-xl mb-5">
            Contact us today and let us help you understand how we manage your data.
          </p>
          <Link
            href="/#contact"
