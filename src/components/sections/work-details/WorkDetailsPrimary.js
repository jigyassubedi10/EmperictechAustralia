...
const WorkDetailsPrimary = () => {
  const worksS = getWork();
  const params = useParams();
  const id = params?.id;
  const currentId = parseInt(id);
  const prevId = currentId > 1 ? currentId - 1 : 1;
  const totalWork = worksS?.length;
  const nextId = currentId < totalWork ? currentId + 1 : totalWork;
  const works = getAWork(currentId);
  const pervWork = getAWork(prevId);
  const nextWork = getAWork(nextId);
  const { title, title2, desc, desc1, desc2, img, descItems, statusItem, slug} =
    works || {};
  const isPrevProject = currentId > 1;
  const isNextProject = currentId < totalWork;
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 gap-x-50px items-start px-15px md:px-25px lg:px-10 mb-10 md:mb-50px">
                      <div>
                        ...
                        <div>
                        <ButtonPrimary className={"group/nested"} url={works?.url || "#"}>
                         live preview
                    ...
                    <div className="px-15px py-30px md:px-25px lg:px-50px lg:py-35px bg-primary-color flex justify-between flex-wrap gap-30px items-center">
                      {/* <!-- prev portfolio --> */}
                      <Link
                        href={isPrevProject ? `/works/${prevId}` : "#"}
                      ...
                      {/* <!-- prev portfolio --> */}
                      <Link
                        href={isNextProject ? `/works/${nextId}` : "#"}
