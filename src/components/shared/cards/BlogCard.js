// components/shared/cards/BlogCard.jsx
"use client";
...
const BlogCard = ({ blog }) => {
  ...
  const { slug, title1, coverImage, shortDesc } = blog;

  return (
    <div className="portfolio-item bg-primary-color-light px-15px pt-25px pb-0 lg:p-9 lg:pb-0 rounded-10px group relative w-full overflow-visible mb-[100px]">
      {/* Image at top */}
      <Image
        src={coverImage}
        alt={title1}
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-[-50px] group-hover:bottom-[-65px] transition-all duration-300 w-[90%]">
        ...
        >
          ...
          <span className="hidden group-hover:block text-sm font-medium underline mt-2 transition-all duration-300">
            Learn more{" "}
            <FaChevronRight className="inline-block ml-1 text-xs sm:text-sm md:text-base lg:text-sm" />
          </span>
        </Link>
      </div>
    </div>
